...
function bbcode2html(str) {
			...
			for(var typeid in smilies_array) {
				for(var page in smilies_array[typeid]) {
					for(var i in smilies_array[typeid][page]) {
						...
						str = str.replace(re, '<img src="../../images/forum/' + smilies_type[typeid][1] + '/' + smilies_array[typeid][page][i][2] + '" border="0" smilieid="' + smilies_array[typeid][page][i][0] + '" alt="' + smilies_array[typeid][page][i][1] + '" />');
					}
				}
			}
		}
	...
	if(!fetchCheckbox('bbcodeoff')) {
		if(allowimgcode) {
			...
			str = str.replace(/\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/ig, function ($1, $2, $3, $4) {return '<img' + ($2 > 0 ? ' width="' + $2 + '"' : '') + ($3 > 2 ? ' height="' + $3 + '"' : '') + ' src="' + $4 + '" border="0" alt="" style="max-width:400px" />'});
		} else {
...
function cuturl(url) {
	var length = 65;
	var urllink = '<a href="#" target="_blank">';
...
function imgtag(attributes) {
	...
	//涓轰簡鍏煎Google娴忚鍣ㄤ笅鍥剧墖灞呬腑鍚庣粰鍔犱笂鐨勮楂�1.5锛屾墍浠ユ妸height澶т簬0鏀逛负浜�2
	return width > 0 || height > 2 ?
