...
function succeedhandle_reply(locationhref, message, param) {
...
//self define confirm
function dconfirm(title, content, callback, src) {
	var time = new Date().getTime();
	var wrapper_id = 'wrapper_' + time;
	var drag_id = 'drag_' + time;

	var wrapper = document.createElement('div');
	 wrapper = jQuery(wrapper);
	jQuery('#append_parent').append(wrapper);
	wrapper.html('<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c" id="fwin_content_">'
			+ '<h3 class="flb" style="cursor:move;"><em class="title"></em><span><a href="javascript:;" class="flbc btn_close" title="关闭">关闭</a></span></h3>'
			+ '<div class="c content" style="clear:both; float:none;"></div><p class="o pns"><a name="confirm" href="javascript:void(0);" class="pn pnc" value="true" style="height:24px; line-height:24px; display:inline-block;"><strong>确定</strong></a></p>'
			+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>');
	wrapper.find('em.title').text(title);
	wrapper.find('div.content').text(content);
	wrapper.addClass('fwinmask');
	wrapper.attr('id', wrapper_id);
	wrapper.find('h3.flb').attr('id', drag_id);

	var btn_close = wrapper.find('a.btn_close');
	var sure = wrapper.find('a[name=confirm]');
	...
	v = {'mtype':'win', 'menuid':wrapper_id, 'duration':3,'pos':'00','zindex':100,'drag':drag_id, 'cache': 0};
	showMenu(v);
	// doane();
}
...
function AC_FL_RunContent() {
	...
	if(AC_DetectFlashVer(9,0,124)) {
		...
		str = '此内容需要 Adobe Flash Player 9.0.124 或更高版本<br /><a href="#" target="_blank"><img src="../../images/forum/get_flash_player.gif" alt="下载 Flash Player" /></a>';
	}
	return str;
}
function AC_GetArgs(args, classid, mimeType) {
	...
	for (var i = 0; i < args.length; i = i + 2){
		...
		switch (currArg){
			case "classid":break;
			case "pluginspage":ret.embedAttrs[args[i]] = '#';break;
			case "src":ret.embedAttrs[args[i]] = args[i+1];ret.params["movie"] = args[i+1];break;
			case "codebase":ret.objAttrs[args[i]] = '';break;
...
function thumbImg(obj, method, value) {
	if(!obj) {
		return;
	}
	obj.onload = null;
	...
	if(method) {
		...
		var imagemaxwidth = isUndefined(imagemaxwidth) ? '583' : imagemaxwidth;
		...
		if(widthary.length > 1) {
			...
			fixw = widthary[0] < value ? value : widthary[0];
		}
		if(zw > fixw) {
			zw = fixw;
			zh = zw / zr;
	...
	if (value > 0 && value <= obj.offsetWidth) {
...
function showColorBox(ctrlid, layer, k) {
	if(!$(ctrlid + '_menu')) {
		...
		menu.style.display = 'none';
		var coloroptions = ['Black', 'Sienna', 'DarkOliveGreen', 'DarkGreen', 'DarkSlateBlue', 'Navy', 'Indigo', 'DarkSlateGray', 'DarkRed', 'DarkOrange', 'Olive', 'Green', 'Teal', 'Blue', 'SlateGray', 'DimGray', 'Red', 'SandyBrown', 'YellowGreen', 'SeaGreen', 'MediumTurquoise', 'RoyalBlue', 'Purple', 'Gray', 'Magenta', 'Orange', 'Yellow', 'Lime', 'Cyan', 'DeepSkyBlue', 'DarkOrchid', 'Silver', 'Pink', 'Wheat', 'LemonChiffon', 'PaleGreen', 'PaleTurquoise', 'LightBlue', 'Plum', 'White', '#585858'];
		var colortexts = ['黑色', '赭色', '暗橄榄绿色', '暗绿色', '暗灰蓝色', '海军色', '靛青色', '墨绿色', '暗红色', '暗桔黄色', '橄榄色', '绿色', '水鸭色', '蓝色', '灰石色', '暗灰色', '红色', '沙褐色', '黄绿色', '海绿色', '间绿宝石', '皇家蓝', '紫色', '灰色', '红紫色', '橙色', '黄色', '酸橙色', '青色', '深天蓝色', '暗紫色', '银色', '粉色', '浅黄色', '柠檬绸色', '苍绿色', '苍宝石绿', '亮蓝色', '洋李色', '白色', '深灰色'];
		var str = '';
		for(var i = 0; i < 40; i++) {
	...
	if(typeof smilies_type == 'object') {
		...
		if(seditorkey == 'fastpost' && $('fastsmilies') && smilies_fast) {
			...
			for(i = 0;i < smilies_fast.length; i++) {
				...
				smilieimg ='../../images/forum/' + smilies_type[smilies_fast[i][0]][1] + '/' + s[2];
				img[k] = new Image();
				img[k].src = smilieimg;
...
function smilies_switch(id, smcols, type, page, seditorkey) {
	...
	for(i in smilies_array[type][page]) {
		...
		smilieimg ='../../images/forum/' + smilies_type[type][1] + '/' + s[2];
		img[k] = new Image();
		img[k].src = smilieimg;
...
function getAppIcon() {
	...
	for(i=0;i<appIcons.length;i++) {
		if(appIcons[i].className=='appicn'){
			appIcons[i].src = '../../images/forum/' + appIcons[i].name;
		}
	}
}
function showUser() {
...
function extstyle(css) {
	...
	$('css_extstyle').href = css ? css + '/style.css' : '#';
	currentextstyle = css;
...
var secST = new Array();
function updatesecqaa(idhash, txttips) {
	txttips = isUndefined(txttips) ? '刷新验证问答' : txttips;
	if($('secqaa_' + idhash)) {
		...
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {$('secqaa_' + idhash).innerHTML = '<span style="cursor:pointer" class="xi2" onclick="updatesecqaa(\''+idhash+'\')">' + txttips + '</span>';}, 180000);
		});
	}
}

function updateseccode(idhash, play, txttips) {
	txttips = isUndefined(txttips) ? '刷新验证码' : txttips;
	if(isUndefined(play)) {
		if($('seccode_' + idhash)) {
			...
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {$('seccode_' + idhash).innerHTML = '<span style="cursor:pointer" class="xi2" onclick="updateseccode(\''+idhash+'\')">' + txttips + '</span>';}, 180000);
			});
		}
	} else {
...
function fixpicwidth(obj, self, system, maxobj) {
	var gwidth = self;
	var maxwidth = maxobj.offsetWidth;
	gwidth = gwidth > maxwidth ? maxwidth : gwidth;
	obj.width = gwidth;
}
