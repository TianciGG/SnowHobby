...
var maxpicnumber = maxpicnumber == undefined ? 50 : parseInt(maxpicnumber);
...
function editorfull(op){
	var op = !op ? 0 : op, control = $(editorid + '_controls'), area = $(editorid + '_textarea').parentNode.parentNode, bbar = $(editorid + '_bbar'), iswysiwyg = wysiwyg;
	if(op) {
...
function ajaxalbumlist(picurl , picurllen) {
	...
	if(message) {
		var attach_regexp = /\[attach\][^\[]+\[\/attach\]/gi;
		var attachimg_regexp = /\[attachimg\][^\[]+\[\/attachimg\]/gi;
		var img_regexp = /\[img[^\]]*\][^\[]+\[\/img\]/gi;
		var attach_matches = message.match(attach_regexp);
		var attachimg_matches = message.match(attachimg_regexp);
		var img_matches = message.match(img_regexp);
		var acount = icount = aicount = 0;
		if(attach_matches) {
			acount = attach_matches.length;
		}
		if(img_matches) {
			icount = img_matches.length;
		}
		if(attachimg_matches) {
			aicount = attachimg_matches.length;
		}
		var allcount = acount + icount + aicount;
		if(allcount > 49) {

			showDialog('您好，为了提高网友的浏览速度，每一楼只限50张照片，其它的照片请以跟贴的形式上传。');
			return;
		}
	}
	if(wysiwyg) {
		insertText('<img src=\''+ picurl +'\' border=0 style=\'max-width:400px\'/>', false)
	} else {
		insertText('[img]'+ picurl +'[/img]', picurllen, 0);
	}
}
function insertAttachimgTag(aid) {
	...
	if(wysiwyg) {
		insertText('<p><img src="' + $('image_' + aid).src + '" border="0" aid="attachimg_' + aid + '" alt="" /></p>', false);
		insertText('<br/><br/>', false);
	} else {
...
function insertAttachimgTagAtZb(aid) {
	var obj = $('postform') && (($('fwin_newthread') && $('fwin_newthread').style.display == '') || ($('fwin_reply') && $('fwin_reply').style.display == '')) ? $('postform') : ($('fastpostform') ? $('fastpostform') : $('postform'));
	...
	if(menu) {
		...
		switch(tag) {
			...
			case 'tbl':
				...
				break;
			case 'aud':
				str = '<p class="pbn">请输入音乐文件地址:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn"><label for="' + ctrlid + '_param_2"><input type="checkbox" id="' + ctrlid + '_param_2" class="pc" value="1"/> 是否自动播放</label><br /></p><p class="xg2 pbn">支持 wma mp3 ra rm 等音乐格式<br />示例: #</p>';
				break;
			case 'vid':
				str = '<p class="pbn">请输入视频地址:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">宽: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; 高: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="pbn"><label for="' + ctrlid + '_param_4"><input type="checkbox" id="' + ctrlid + '_param_4" class="pc" value="1"/> 是否自动播放</label><br /></p><p class="xg2 pbn">支持优酷、土豆、56、酷6等视频站的视频网址<br />支持 wmv avi rmvb mov swf flv 等视频格式<br />示例: #</p>';
				break;
			case 'fls':
				str = '<p class="pbn">请输入 Flash 文件地址:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">宽: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; 高: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">支持 swf flv 等 Flash 网址<br />示例: #</p>';
				break;
			default:
