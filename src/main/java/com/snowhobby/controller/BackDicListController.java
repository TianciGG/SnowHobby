...
public class BackDicListController {
	@Autowired
	...
	public List<DicCode> selectDicType() {
		return dicListService.selectDicType();
	}

	// 数据字典管理页面，页面加载、单击查询按钮。。。时分页显示相关查询数据的请求
	@RequestMapping(value = "/getAllDic.do")
	@ResponseBody
	public List<DicCode> getAllDic(DicCode dicCode, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		request.setCharacterEncoding("UTF-8");
		response.setCharacterEncoding("UTF-8");
		return dicListService.selectDicAll(dicCode);
	}
	...
	public DicCode getOneDicCode(DicCode dicCode) {
		return dicListService.selectByPrimaryKey(dicCode.getDicId());
	}

	// 数据字典管理页面，添加按钮和分页操作项的编辑图标打开页面的确定按钮产生的请求
	@RequestMapping(value = "/addOrUpdate.do")
	@ResponseBody
	public void addOrUpdate(DicCode dicCode, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		response.setCharacterEncoding("UTF-8");
		PrintWriter pw = response.getWriter();
		if (!"".equals(dicCode.getDicId())) {
			pw.write(dicListService.updateByPrimaryKeySelective(dicCode) == true ? "allok"
					: "myfail");
		} else {
			dicCode.setDicId(UUID.randomUUID().toString());
			dicCode.setDicCode(UUID.randomUUID().toString());
			pw.write(dicListService.insertSelective(dicCode) == true ? "allok"
					: "myfail");
		}
		pw.flush();
		pw.close();
	...
	@ResponseBody
	public void deleteOneDicCode(DicCode dicCode, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		response.setCharacterEncoding("UTF-8");
		PrintWriter pw = response.getWriter();
		pw.write(dicListService.deleteByPrimaryKey(dicCode.getDicId()) == true ? "allok"
				: "myfail");
		pw.flush();
		pw.close();
	}
	...
	@ResponseBody
	public List<DicCode> checkOriginalPwd(DicCode dicCode,
			HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		response.setCharacterEncoding("UTF-8");
		return dicListService.getDataByType(dicCode.getType());
	}
}
